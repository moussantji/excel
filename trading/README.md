...
| **Trading Game — Simulateur Bourse** | Simulateur sur de vrais cours, plus de 400 questions notées, entraîneur de figures, leçons courtes | Gratuit + achats dans l'application (4,6 / 25 000 avis) · français | Version gratuite limitée, et l'application pousse vers un courtier partenaire : gardez les quiz, pas l'invitation à déposer |
...
**Mise en garde écrite dans l'application** : une grande partie des applications « trading » du Play Store sont des **vitrines de courtiers** qui poussent au dépôt d'argent réel (certaines accumulent les avis de retraits en échec). Même une application éducative sérieuse peut pousser vers son courtier partenaire. Aucune n'est nécessaire pour s'entraîner ; pour passer des ordres sans risque, un **compte démo** chez un courtier régulé suffit.
...
Sept figures, dans la vue Formation :
...
| 6 | **Le retest de la séquence 1** : le retour du prix sur le niveau cassé (4 312,9), jusqu'à **4 303,7** à 14:15, puis la reprise à **4 317,0** — le stop descend à 13,3 points |
| 7 | **Le rejet de la séquence 2** : le prix remonte et **s'arrête 1,2 point sous le niveau cassé** (4 364,5 à 07:00) — l'ordre posé sur le niveau n'aurait jamais été rempli |
...
Recette dédiée : `node tools/etude-ltf-test.js` (**144 contrôles** — intégrité des 96 créneaux de 15 minutes et de chaque bougie, trous de cotation, faits de la séance, fenêtres de tir comparées à celles de `plan.js`, les **deux séquences relues bougie par bougie sans passer par la carte** (le balayage dépasse bien le niveau attendu, la bougie y revient, le ChoCh est la *première* clôture qui casse l'extrême formé depuis le balayage, l'entrée est cette clôture, le stop n'est jamais touché avant la sortie, la sortie est l'extrême des seize bougies suivantes), arithmétique du risque et du multiple, refus du plan chiffrés (2,07 % et 1,68 % du capital, 1:7 impossible), dessin SVG (bougies, repères, aucune étiquette vide, qui se chevauche ou qui sort du cadre, aucune pastille sur une étiquette), **le retest recalculé bougie par bougie** (niveau cassé, point extrême du retour, première clôture qui repart, stop qui en découle, les deux stops sous les 15 points du plan, le niveau jamais touché à la vente, l'invalidation par clôture au-delà du balayage), cache hors ligne, rendu dans la vue Formation et ouverture du graphique en 15 minutes).

#### Comprendre : le ChoCh, puis le retest

Le plan est précis : « j'attends la prise de liquidité, puis le ChoCh en LTF », et « j'entre au retest de la zone avec un stop serré ». La page déplie les deux notions, chiffres en main.

- **Le ChoCh n'est pas un signal d'entrée, c'est un constat** : la première clôture qui casse le dernier extrême formé depuis la bougie de balayage. Une mèche qui dépasse ne suffit pas ; et avoir raison sur la direction ne dit pas que le prix est au bon prix — sur la clôture du ChoCh, le stop est derrière le balayage, très loin.
- **Le retest, c'est le retour du prix sur le niveau qu'il vient de casser.** C'est lui qui resserre le stop, et rien d'autre.
- **Séquence 1 (achat)** : le ChoCh casse **4 312,9** à 13:15 ; le prix revient dès 13:45, puis descend jusqu'à **4 303,7** à 14:15 (9,2 points sous le niveau, toujours au-dessus du balayage) et repasse au-dessus à 14:30 (clôture **4 317,0**). Le stop derrière ce point bas ne fait plus que **13,3 points** au lieu de 20,7. En posant l'ordre sur le niveau cassé, **9,2 points** seulement — mais le prix ne revient pas toujours jusque-là.
- **Séquence 2 (vente)** : le ChoCh casse **4 365,7** à 06:45 ; le prix remonte mais **s'arrête 1,2 point sous le niveau** (4 364,5 à 07:00) : un ordre posé sur le niveau **n'aurait jamais été rempli**. L'entrée est la clôture de 07:15 (**4 357,3**), stop derrière le rejet (**4 364,5**) : **7,2 points** au lieu de 16,8.
- **Ce qui annule un ChoCh** n'est pas le retour sur le niveau, mais une **clôture de l'autre côté de l'extrême du balayage** (sous 4 293,0 à l'achat, au-dessus de 4 379,3 à la vente). Sur cette séance, aucune des deux lectures n'a été invalidée avant la sortie.
- **Le même trade, deux entrées** : un tableau compare, pour chaque séquence, la clôture du ChoCh et le retest — prix, stop, risque, règle des 15 points, coût sur 1 000 $ au plus petit lot, capital nécessaire, multiple. Les deux stops du retest passent **sous les 15 points du plan** (13,3 et 7,2) là où aucune des deux entrées au ChoCh ne les respectait (20,7 et 16,8) ; celui de la séquence 2 passe même sous 1 % d'un compte de 1 000 $ (**0,72 %**, soit 720 $).
- **Et ce que le retest ne change pas** : il ne fabrique pas un trade. Aucune des deux séquences n'atteint le 1:7 du plan (3,2 et 5,6 fois le risque au mieux), la seconde reste hors des fenêtres de tir, et le retest n'est jamais garanti — quand le prix part sans revenir, il ne reste que la première entrée, la plus large, ou le renoncement.

Deux figures de plus (sept au total, toutes dessinées par l'application) montrent le retour et le rejet, avec le niveau cassé, le point extrême du retest et le nouveau stop.
...
- **Le mois entier en grille** : chaque case porte son numéro et une petite barre de progression — verte (complète), dorée (entamée), vide (rien de coché). Le dimanche est signalé, aujourd'hui est encadré, les jours à venir sont grisés. Un appui sur une case affiche cette journée.- **Les bilans** : jours complets du mois sur les jours écoulés (et le pourcentage), jours entamés, **jours ouvrés complets de la semaine**, et **la série en cours** de jours complets d'affilée. Une journée entamée mais non finie arrête la série ; une journée pas encore commencée laisse compter jusqu'à hier — on n'est pas puni avant d'avoir commencé.
...
- **Un accès direct à l'exemple en basse unité de temps** : sous les moments, un lien « Voir cet exemple en 15 minutes » ouvre la Formation sur la **carte de la séance en 15 minutes** (avec un cadre doré le temps de la repérer). La carte de la routine **ne recopie rien** : l'exemple vit à un seul endroit.
