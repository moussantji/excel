...
(async function () {
  ...
  const vCacheSync = (fs.readFileSync(path.join(RACINE, 'sw.js'), 'utf8').match(/const VERSION = 'trading-desk-v(\d+)'/) || [])[1];
  verif('version de cache du service worker déclarée', !!vCacheSync, vCacheSync ? 'v' + vCacheSync : 'absente');
  verif('cache au moins v8 (verrou + rappels inclus)', Number(vCacheSync) >= 8);
